...
1) 3번(타이포·정렬): 위아래로 쌓인 제목·부제목처럼 서로 정렬이 의도된 것으로 보이는 텍스트들을 비교하기 전에, 먼저 이 텍스트들이 가운데 정렬인지 좌측 정렬인지 우측 정렬인지 시각적으로 판단하세요. 판단한 정렬 방식에 맞는 좌표만 비교하세요:
- 가운데 정렬로 보이면: 가로중심끼리 비교
- 좌측 정렬로 보이면: 좌측 좌표끼리 비교
- 우측 정렬로 보이면: 우측 좌표끼리 비교
좌측 정렬이나 우측 정렬인 텍스트에는 가로중심을 비교하지 마세요 — 좌우 정렬된 소재에 중앙정렬 기준(가로중심)을 적용하면 실제로는 문제없는 디자인도 어긋난 것처럼 보이고, note를 읽는 사람도 왜 가로중심을 따지는지 헷갈립니다. note에도 실제로 비교한 기준(가로중심/좌측/우측)을 명시하세요. 이렇게 정렬 방식을 정한 뒤, 해당 좌표를 기준으로 정확히 몇 px 차이나는지 계산하세요.
