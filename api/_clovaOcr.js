...
나) 캔버스 기준 중앙정렬: 로고, 헤드라인, CTA처럼 이미지 전체 폭 안에서 가운데 배치된 것으로 보이는 요소는, 왼쪽 여백(요소의 좌측 좌표 - 0)과 오른쪽 여백(위에 안내된 이미지 원본 가로 크기 - 요소의 우측 좌표)을 각각 계산해서 비교하세요. 위아래로 쌓인 여러 요소를 함께 비교할 때뿐 아니라, 로고 하나처럼 단독으로 놓인 요소도 캔버스 중앙에서 벗어나 있는지 반드시 확인하세요 — 가) 항목은 "요소끼리" 비교이고, 나) 항목은 "요소 대 캔버스 전체" 비교라 서로 다른 체크입니다. 이 비교도 가)와 동일한 기준을 적용하세요: 좌우 여백 차이가 0px이면 pass, 1~2px 차이면 needsfix, 3px 이상 차이면 reject로 판정하고 note에 "좌측 여백 OOpx, 우측 여백 OOpx로 캔버스 중앙에서 벗어남"처럼 구체적인 여백 수치를 적으세요.
