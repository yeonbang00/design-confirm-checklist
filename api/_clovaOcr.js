...
다) CTA 버튼의 정렬: CTA처럼 도형(배경) 안에 텍스트가 들어간 요소는 위 OCR 좌표가 텍스트 자체의 경계일 뿐, 버튼 도형의 실제 테두리가 아닐 수 있습니다 — 버튼에 좌우 패딩이 있으면 텍스트 좌표만으로 캔버스 여백(나)이나 다른 요소와의 정렬(가)을 계산하면 실제 버튼 위치와 오차가 생깁니다. CTA에 가)·나) 기준을 적용할 때는 OCR 좌표를 출발점으로만 삼고, 반드시 이미지를 직접 보고 버튼 도형 자체의 좌우 여백까지 시각적으로 확인해서 최종 판단하세요. 또한 CTA 문구 옆에 붙은 화살표(>, ›, →), 꺾쇠, 아이콘, 장식 기호는 OCR이 아예 잡지 못하거나 문구와 분리된 별개 덩어리로 잡는 경우가 많습니다 — 그러면 OCR 좌표가 눈에 보이는 CTA 덩어리보다 좁아져서, 실제로는 화살표까지 포함해 가운데 맞춰진 CTA가 좌측으로 몇 px 치우친 것처럼 계산됩니다. 디자이너는 이런 기호까지 하나의 덩어리로 보고 정렬하므로, CTA에 기호·아이콘이 붙어 있으면 OCR 좌표만으로 정렬을 단정하지 말고 기호·아이콘까지 포함한 덩어리 전체를 이미지에서 눈으로 확인해 판단하세요. 이 주의는 CTA뿐 아니라 아이콘과 텍스트가 나란히 놓인 모든 요소에 똑같이 적용됩니다.
