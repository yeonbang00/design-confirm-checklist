...
비교하는 글자·도형에 뾰족하거나 기울어진 형태(ㅅ, ㅈ, ㅊ 등)와 둥근 형태(ㅇ)처럼 시각적으로 비대칭적인 모양이 섞여 있어 의도적인 "광학적 정렬"이 의심되는 경우는, 위 표의 등급을 완전히 무시하는 예외가 아니라 등급을 한 단계 낮추는 참고 사유로만 쓰세요. 단, 이 경우의 허용 오차는 고정된 px 수치가 아니라 비교하는 텍스트의 실제 높이(하단 좌표 - 상단 좌표)에 비례해서 판단하세요 — 폰트 크기가 클수록 같은 광학보정이라도 절대 px 차이가 커지는 게 정상이라, 작은 텍스트와 큰 헤드라인에 같은 px 기준을 적용하면 안 됩니다. 차이를 텍스트 높이로 나눈 비율을 계산해서: 텍스트 높이의 약 5% 이내 차이는 광학적 정렬로 설명 가능한 정상적인 수준이니 pass로 낮추지 말고 최소 needsfix로 판정하고, 5%를 초과하는 차이는 광학적 정렬로 설명하기엔 너무 크므로 이 예외를 적용하지 말고 reject로 판정하세요. (예: 텍스트 높이가 약 100px일 때 4px 차이는 4%로 정상 범위지만, 텍스트 높이가 약 24px일 때 4px 차이는 약 17%로 과도한 차이입니다.) 광학적 정렬 요소가 섞이지 않은 일반 비교(사각형·직선처럼 대칭적인 형태끼리)에는 이 비율 기준을 적용하지 말고 위의 고정 0/1~2/3px 기준을 그대로 적용하세요.

note에 어느 요소끼리 몇 px 차이나는지 구체적으로 적으세요 (예: "'부하우스'와 '압축파우치·롤링캐리백' 가로중심이 4px 어긋남"). 광학적 정렬 가능성 때문에 needsfix로 낮춰 판정했다면, note에 "OCR상 OOpx 차이가 있으나 광학적 정렬 영향으로 추정, 확인 권장"처럼 그 이유를 남기세요 — "정렬 맞음"처럼 문제가 없다고 단정하는 표현은 쓰지 마세요.

나) 캔버스 기준 중앙정렬: 로고, 헤드라인, CTA처럼 이미지 전체 폭 안에서 가운데 배치된 것으로 보이는 요소는, 왼쪽 여백(요소의 좌측 좌표 - 0)과 오른쪽 여백(위에 안내된 이미지 원본 가로 크기 - 요소의 우측 좌표)을 각각 계산해서 비교하세요. 위아래로 쌓인 여러 요소를 함께 비교할 때뿐 아니라, 로고 하나처럼 단독으로 놓인 요소도 캔버스 중앙에서 벗어나 있는지 반드시 확인하세요 — 가) 항목은 "요소끼리" 비교이고, 나) 항목은 "요소 대 캔버스 전체" 비교라 서로 다른 체크입니다. 이 비교도 가)와 동일한 기준을 엄격하게 적용하세요 — 3px 차이는 육안으로도 확인되는 수준이니 봐주지 마세요: 좌우 여백 차이가 0px이면 pass, 1~2px 차이면 needsfix, 3px 이상 차이면 reject로 판정하고 note에 "좌측 여백 OOpx, 우측 여백 OOpx로 캔버스 중앙에서 벗어남"처럼 구체적인 여백 수치를 적으세요. 단, 이 요소에 스트록(외곽선)이 적용돼 있거나 커스텀 서체·장식이 섞인 로고 워드마크처럼 OCR이 경계를 안정적으로 잡기 어려워 보이는 경우는, 가)의 스트록 규칙과 동일하게 이 px 표를 적용하지 말고 이미지를 직접 보고 육안으로만 캔버스 중앙정렬 여부를 판단하세요.
