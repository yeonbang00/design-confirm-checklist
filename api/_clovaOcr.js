...
- 스트록·그림자 효과 확인: 비교하는 텍스트에 외곽선(스트록)이나 그림자 효과가 적용되어 있다면, OCR이 측정하는 좌우 경계는 실제 글자 획뿐 아니라 스트록·그림자 픽셀까지 포함해서 늘어난 값일 수 있습니다. 다만 이 효과로 부풀려질 수 있는 오차는 보통 몇 px 수준입니다 — 차이가 10px처럼 크게 나온다면 스트록·그림자만으로는 설명되지 않는 크기이니, 이 경우는 예외로 보지 말고 아래 기준대로 그대로 판정하세요.

대응 관계가 확실한 비교라면, 비교하는 두 요소가 사각형·직선처럼 대칭적인 형태일 때 아래 기준을 적용하세요:
- 0px(완전 일치): pass
- 1~2px 차이: needsfix
- 3px 이상 차이: reject

스트록·그림자 효과나, 비교하는 글자·도형에 뾰족하거나 기울어진 형태(ㅅ, ㅈ, ㅊ 등)와 둥근 형태(ㅇ)처럼 시각적으로 비대칭적인 모양이 섞여 있어 의도적인 "광학적 정렬"이 의심되는 경우는, 위 표의 등급을 완전히 무시하는 예외가 아니라 등급을 한 단계 낮추는 참고 사유로만 쓰세요 — 1~2px 수준의 작은 차이가 이런 이유로 생긴 것으로 보이더라도 pass로 낮추지 말고 최소 needsfix로 판정하세요. 3px 이상처럼 스트록·그림자·광학적 정렬로 설명하기엔 너무 큰 차이는 이 예외를 적용하지 말고 reject로 판정하세요.

note에 어느 요소끼리 몇 px 차이나는지 구체적으로 적으세요 (예: "'부하우스'와 '압축파우치·롤링캐리백' 가로중심이 4px 어긋남"). 스트록·그림자나 광학적 정렬 가능성 때문에 needsfix로 낮춰 판정했다면, note에 "OCR상 OOpx 차이가 있으나 스트록/그림자·광학적 정렬 영향으로 추정, 확인 권장"처럼 그 이유를 남기세요 — "정렬 맞음"처럼 문제가 없다고 단정하는 표현은 쓰지 마세요.

나) 캔버스 기준 중앙정렬: 로고, 헤드라인, CTA처럼 이미지 전체 폭 안에서 가운데 배치된 것으로 보이는 요소는, 왼쪽 여백(요소의 좌측 좌표 - 0)과 오른쪽 여백(위에 안내된 이미지 원본 가로 크기 - 요소의 우측 좌표)을 각각 계산해서 비교하세요. 위아래로 쌓인 여러 요소를 함께 비교할 때뿐 아니라, 로고 하나처럼 단독으로 놓인 요소도 캔버스 중앙에서 벗어나 있는지 반드시 확인하세요 — 가) 항목은 "요소끼리" 비교이고, 나) 항목은 "요소 대 캔버스 전체" 비교라 서로 다른 체크입니다. 단, 이 비교는 한 요소의 좌측·우측 경계를 각각 독립적으로 측정해서 빼는 방식이라 가)보다 OCR 측정 오차가 누적되기 쉽습니다 — 특히 커스텀 서체·장식이 섞인 로고 워드마크는 OCR이 글자 경계를 잡을 때 몇 px씩 흔들릴 수 있습니다. 그러니 이 기준을 적용하세요: 좌우 여백 차이가 0~2px이면 pass, 3~6px이면 needsfix(OCR 측정 오차일 가능성이 있으니 확정적 반려는 피하고, note에 "OCR상 차이가 있으나 측정 오차 가능성도 있어 확인 권장"처럼 남기세요), 7px 이상 차이면 reject로 판정하고 note에 "좌측 여백 OOpx, 우측 여백 OOpx로 캔버스 중앙에서 벗어남"처럼 구체적인 여백 수치를 적으세요.
