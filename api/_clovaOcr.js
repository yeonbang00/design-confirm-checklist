...
비교하는 글자·도형에 뾰족하거나 기울어진 형태(ㅅ, ㅈ, ㅊ 등)와 둥근 형태(ㅇ)처럼 시각적으로 비대칭적인 모양이 섞여 있어 의도적인 "광학적 정렬"이 의심되는 경우는, 위 표의 등급을 완전히 무시하는 예외가 아니라 등급을 한 단계 낮추는 참고 사유로만 쓰세요 — 1~2px 수준의 작은 차이가 이런 이유로 생긴 것으로 보이더라도 pass로 낮추지 말고 최소 needsfix로 판정하세요. 3px 이상처럼 광학적 정렬로 설명하기엔 너무 큰 차이는 이 예외를 적용하지 말고 reject로 판정하세요.
