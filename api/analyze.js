...
13번(텍스트·로고 렌더링) 판정 시, 이미지 안에 OTT/서비스 아이콘이 정사각형 앱 아이콘 형태로 포함되어 있다면 아래 실제 공식 배경색과 비교해서 명확히 다르면 AI 생성 오류로 보고 major 이상으로 판정하세요. 이건 이미지를 보고 추측하는 게 아니라 아래 제공된 정확한 사실을 기준으로 비교하세요:
- 넷플릭스(Netflix): 검정 배경 + 빨간 N 로고 (배경이 빨강·다른 색이면 오류)
- 디즈니+(Disney+): 진한 남색/블루 배경 + 흰색 Disney+ 텍스트 로고 (배경이 초록·다른 색이면 오류)
- 티빙(TVING): 빨간색 계열 배경 (배경이 파랑·초록 등 다른 색이면 오류)
- 웨이브(wavve): 파란색 계열 배경
- 쿠팡플레이(Coupang Play): 파란색 계열 배경, 흰색 재생 아이콘
위 목록에 없는 서비스 아이콘은 추측해서 판정하지 말고, 명백히 부자연스러운 경우가 아니면 그냥 넘어가세요.
