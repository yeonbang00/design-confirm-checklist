...
크롭 관련 주의는 메타(릴스 및 스토리)처럼 실제로 기기별 화면비율에 따라 자동 크롭이 발생하는 매체를 선택했고, 그 매체 가이드에 크롭 관련 안내가 있는 경우에만 언급하세요. 매체를 선택하지 않았거나 선택한 매체 가이드에 크롭 관련 언급이 없다면, 크롭 위험을 임의로 지어내서 언급하지 마세요 — 대부분의 게재 매체는 고정 사이즈로 그대로 노출되고 자동으로 잘리지 않습니다.
...
- satisfied: 시안이 매체 기준을 충족하는 부분과 그 근거를 2~3문장(40~60단어)으로 설명 (예: "메타(릴스 및 스토리) 기준으로는 로고가 세이프존 안에 잘 들어와 있다")
