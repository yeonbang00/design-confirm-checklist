...
// 18 items = 10 core design/strategy checks + 4 AI-generation artifact checks
...
const BASE_PROMPT = `다음은 광고대행사가 광고주에게 전달하기 전 최종 검수하는 배너 시안 이미지입니다. 목적은 광고주 전달 후 발생할 수정 요청을 사전에 줄이는 것입니다. 아래 18개 항목을 기준으로 평가하세요.

1 전략 적합성 - 캠페인 목적/타깃 톤앤매너 일치, 여러 혜택 포인트가 하나의 오퍼로 수렴하는지 (서로 다른 오퍼·캠페인이 경쟁하지 않는지)
2 위계 및 구조 - 헤드라인→서브→CTA 시선 동선, 3초 내 파악 가능 여부
3 타이포·정렬 - 폰트 2~3종 이내, 요소 간 상대 정렬과 캔버스 기준 중앙정렬, CTA 도형·내부 텍스트 정렬, 강조가 필요한 핵심 정보의 시각적 구분
4 정보 정확성 - 가격/날짜/단위 등 오탈자 여부
5 컬러 일관성·대비 - 브랜드 컬러 준수, 색상 3~4개 제한, 대비
...
10 여백·간격 - 요소 간 여백·간격의 일관성, 텍스트 줄바꿈 위치의 자연스러움
...
18 법적고지·심의 문구 - 금융/대출/의료/주류/게임 등 규제 업종에 필요한 고지·심의 문구 포함 여부 (해당 업종이 아니면 na)

18개 항목은 pass/needsfix/reject/na 중 하나로 판정하세요.
- pass: 문제 없음
...
- reject: 광고주 전달 전 무조건 확인·수정해야 하는 문제입니다. "시안을 통째로 다시 만들어야 할 정도"라는 뜻이 아니라, 고치는 것 자체는 쉽더라도 반드시 짚고 넘어가야 하는 항목이라는 뜻입니다. 아래 두 기준 중 하나에 해당하면 reject로 판정하세요:
  1) 3번(정렬 관련 부분)·4번·10번·17번에서 발견된 문제는 크기와 무관하게 무조건 reject로 판정하세요(needsfix로 낮추지 마세요) — 오탈자, 여백·정렬, 로고 사용 규정처럼 객관적으로 확인 가능한 필수 준수 사항이기 때문입니다.
  2) 그 외 항목(1,2,5,6,7,8,9,11,12,13,14,16)은 문제가 대부분의 사람이 봐도 명백하게 눈에 띄는 저퀄리티(AI 생성 아티팩트가 뚜렷하거나 합성이 확연히 부자연스러운 경우 등)일 때만 reject로 판정하고, 미묘하거나 선택적으로 더 다듬으면 좋은 수준이라면 needsfix로 판정하세요.
15번(매체 최적화)은 매체 선택 여부에 따라 기준이 다릅니다 — 매체를 선택해서 구체적인 등록 기준(세이프존 %, 사이즈 등)이 제공된 경우, 그 기준을 명확하고 크게 벗어났다면 reject로 판정하세요. 반면 매체를 선택하지 않아 "세이프존, 모바일 가독성"이라는 일반 기준만 있는 경우에는, 구체적인 수치 기준 없이 AI 스스로의 판단("여백이 부족해 보인다" 등)에 의존하는 것이므로 reject로 단정하지 말고 needsfix로 판정하세요 — 디자인 선택의 여지를 존중하되, 개선하면 좋을 부분으로만 짚어주세요.
...
18번(법적고지·심의 문구)은 예외입니다 — 이미지만 보고 어떤 문구가 정확히 필요한지, 표기가 규정에 맞는지까지는 확신할 수 없으므로 reject로 단정하지 마세요. 고지 문구가 아예 안 보이거나 너무 작아 알아보기 어려운 경우 needsfix로 판정하고, note에 "정확한 필요 문구·표기 기준은 담당자 확인 필요"라고 남기세요. 문구가 있고 육안으로 특별히 이상해 보이지 않으면 pass로 판정하세요.
- na: 해당 시안에 명백히 적용되지 않는 항목 (예: 인물 없는 시안의 11번, 비규제 업종의 18번)
...
10번(여백·간격)을 판정할 때는 오탈자·정렬이 아니라 다음 두 가지만 확인하세요:
...
13번(워터마크·서명 흔적)을 판정할 때 중요한 예외: "AI로 생성된 이미지입니다", "AI로 생성된 제작물", "Made with AI", "AI-generated", "* AI 생성" 등 AI 생성 사실을 알리는 고지 문구(한글·영문 표현 모두 포함)는 13번이 잡으려는 "결함"이 아닙니다 — 오히려 의도적으로 넣은 필수 고지 문구입니다. 이런 문구가 이미지 구석에 작게 적혀 있다고 해서 13번을 reject나 needsfix로 판정하지 마세요. 13번은 원래 목적대로, 광고주가 의도하지 않은 스톡 이미지 사이트의 워터마크(예: Shutterstock, Getty Images 로고)나 생성 도구가 실수로 남긴 서명·로고(예: 의도치 않게 찍힌 "Midjourney" 같은 텍스트)가 남아있는지만 확인하세요.

17번(로고 사용 규정)을 판정할 때, 이미지 전체를 꼼꼼히 살펴 브랜드 로고나 워드마크가 어디에도 없다면 na로 넘어가지 말고 reject로 판정하세요 — note에 "로고가 보이지 않음 — 브랜드 로고 누락 여부 확인 필요"라고 남기세요. 로고가 있는데 여백·변형 등에 문제가 없다면 pass로 판정하세요.

17번의 "최소 여백" 판단에는 정해진 px 기준이 없습니다 — 로고가 다른 요소(텍스트·다른 그래픽·이미지 가장자리)와 부자연스럽게 겹치거나 바짝 붙어 보이지는 않는지, 반대로 너무 작거나 크게 배치돼 어색해 보이지는 않는지를 순전히 육안으로만 판단하세요. 특정 px 수치를 근거로 들지 마세요 — "N px 미달" 같은 표현은 쓰지 말고, 여백이 부자연스러우면 note에 "로고가 OO 요소와 너무 가깝게 배치됨"처럼 구체적으로 어디가 어떻게 부자연스러운지 서술하세요. (참고: 3번의 "캔버스 기준 중앙정렬" 규칙에 나오는 10px 기준은 로고가 캔버스 좌우 중앙에서 벗어났는지를 재는 별개의 정렬 판정이니, 17번의 여백 판정과 혼동하지 마세요.)

5번(컬러 일관성·대비)을 판정할 때는 다음을 확인하세요:
...
4) 색상 조합 제안: 지금 배색이 눈에 잘 띄지 않거나 개선 여지가 있다고 판단되면, 아래 참고 배색 중 배너 톤에 맞는 조합을 구체적으로 언급하며 제안하세요. 다만 "이렇게 바꾸세요"처럼 단정하지 말고 "이런 계열도 고려해볼 만하다"는 참고용 제안으로 표현하세요 — 최종 색상 선택은 디자이너의 몫입니다. 브랜드 컬러가 지정된 시안이라면 그 브랜드 컬러 범위 안에서만 제안하세요.
...
16번(텍스트·로고 렌더링)을 판정하기 전에, 반드시 아래 절차를 순서대로 실행하세요:
...
2) 앱 아이콘 확인(정사각형 둥근 모서리 앱 아이콘이 있는 배너에만 해당): 이미지 안에 정사각형(둥근 모서리) 앱 아이콘이 있다면 몇 개인지, 각각 어떤 서비스인지 나열하세요. 앱 아이콘이 하나도 없다면 이 단계는 그냥 건너뛰고, "앱 아이콘 없음" 같은 문구를 note에 남기지 마세요 — 앱 아이콘 유무는 16번 판정과 무관한 정보입니다.
...
5) 배경색이 위 정답과 명확히 다르면(예: 넷플릭스가 빨간 배경, 디즈니+가 순수 블루나 보라색 등) 16번을 최소 검토필요로 판정하세요. 오류가 발견된 아이콘이 여러 개면 메모에 전부 짧게 나열하세요 (예: "넷플릭스·디즈니+ 배경색 오류"). 하나만 골라서 적지 마세요.
5) 목록에 없는 서비스 아이콘은 추측해서 판정하지 말고 넘어가세요.

11·12·14번(AI 생성 아티팩트)을 판정하기 전에도 인상만 보고 넘어가지 말고 아래 절차를 실행하세요:
...
7번(피사체 보정)과 11번(신체 비율 왜곡)은 서로 다른 문제를 봅니다 — 같은 부자연스러운 부분이라도 아래 기준으로 구분해서 하나의 항목에만 명확히 배정하세요, 두 항목에 같은 내용을 중복해서 적지 마세요:
- 7번은 "질감·보정 품질" 문제입니다: 피부·재질이 과도하게 매끈하거나 플라스틱처럼 보이는 등, 형태 자체는 정상인데 표면 텍스처만 부자연스러운 경우.
- 11번은 "형태·비율" 문제입니다: 손가락 개수·팔다리 길이·얼굴 이목구비 위치처럼 구조 자체가 물리적으로 말이 안 되는 경우.
같은 부분에서 질감 문제와 형태 문제가 동시에 보이면(예: 손 모양도 뒤틀리고 질감도 부자연스러움) 각각 7번과 11번에 따로 적으세요.
...
function mediaGuidelineInstruction(mediaGuides, hasSize) {
  ...
  let text = `\n\n매체 가이드 안내: 이 시안은 다음 매체에 게재될 예정입니다: ${names}. 15번(매체 최적화) 항목을 판정할 때 아래 각 매체 기준을 모두 반영하세요.${sizeLine}
...
function imageSizeInstruction(width, height, fileSizeBytes) {
  ...
  const capacityLine = sizeLabel
    ...
    : '';
  return `\n\n이미지 크기 정보: 업로드된 이미지의 실제 원본 크기는 정확히 ${width} x ${height}px입니다.${capacityLine} 이는 프로그램이 파일에서 직접 측정한 정확한 값이니, 이미지를 보고 크기나 용량을 다시 추측하지 말고 이 값을 그대로 사용하세요. 15번(매체 최적화) 항목과 매체 가이드 판정 시 이 정확한 크기·용량을 기준으로 삼으세요.`;
}
...
5번(컬러 일관성·대비)을 판정할 때 이 수치를 참고하세요. 4.5:1 미만이면 WCAG 기준상 가독성이 부족한 것으로 보고, 특히 3:1 미만처럼 명확히 낮은 값은 needsfix 이상으로 판정하세요. 다만 이 수치는 근사치이니 애매한 값(3.5~4.5 사이 등)이면 실제로 눈으로 봤을 때도 읽기 불편한지 시각적으로 한 번 더 확인해서 판단하세요.
...
export default async function handler(req, res) {
  ...
  // 클라이언트가 5번(컬러 일관성·대비)의 명도 대비를 직접 계산하려면 먼저 OCR
  ...
  try {
    const parsed = await callOpenAI({
      ...
      // 오탈자 확인(4번)이 팀 최우선 기준인데, effort가 낮으면 18개
