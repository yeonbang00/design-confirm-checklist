...
CTA 버튼이 있다면 손가락으로 누르기에 지나치게 작아 보이는지도 확인하세요 — 모바일 환경 기준 최소 탭 영역은 대략 44×44px(웹 접근성 표준의 일반적인 권고치)인데, OCR로는 버튼 도형의 실제 크기를 정확히 잴 수 없으니 px 수치를 근거로 들지 말고 이미지 전체 크기에 비춰 버튼이 눈에 띄게 작아 보이는지 육안으로만 판단하세요. 명백히 작아서 누르기 불편해 보일 때만 note에 참고 제안으로 남기고(예: "CTA 버튼이 작아 모바일에서 누르기 불편할 수 있습니다"), needsfix 이상으로 올리지는 마세요 — 크기 자체는 매체가 별도로 버튼을 감싸는 경우도 많아 단정할 수 없는 부분입니다.

CTA 문구 길이도 참고하세요: 헤드라인·서브카피에 이미 긴급성·혜택(예: "3일한정", "특가")이 명확히 담겨 있는데도 CTA 문구가 "3일한정! 참여하고 특가혜택받으러가기 >"처럼 길게 그 내용을 반복한다면, note에 참고 제안으로 "CTA가 길어 헤드라인과 내용이 겹칩니다 — CTA는 짧게 줄이고 긴급성은 헤드라인 쪽에 맡기는 게 낫습니다"처럼 남기세요. 판정 등급을 올리지는 말고, 헤드라인에 긴급성이 없어서 CTA가 그 역할까지 하고 있는 것으로 보이는 경우에만 이 제안을 하세요.

CTA 문구가 "문의하기"·"제출"·"클릭"처럼 눌렀을 때 뭘 얻는지 알 수 없는 사무적이고 모호한 단어 하나로만 돼 있다면, note에 참고 제안으로 "CTA 문구가 무엇을 얻는지 알려주지 않습니다 — '무료로 상담받기'·'혜택받기'처럼 행동의 결과를 짧게라도 담아보세요"처럼 남기세요. 이건 위 CTA 길이 규칙과 반대 방향의 문제입니다 — 헤드라인 내용을 반복해서 길어지는 게 아니라, 반대로 아무 정보 없이 짧기만 해서 문제인 경우이니 혼동하지 마세요. "구매하기"·"혜택받기"·"신청하기"처럼 짧으면서도 결과가 명확한 CTA는 이 지적 대상이 아닙니다. 또한 "무료"·"지금 바로"처럼 행동 부담을 낮추는 단어가 있으면 클릭 유도에 도움이 되니, 이런 단어 없이 딱딱하게만 쓰여 있다면 가볍게 참고 제안할 수 있습니다. 판정 등급은 올리지 마세요.
...
- needsfix·reject: 무엇이·어디가 문제인지 정확히 전달하는 데 필요한 만큼 쓰세요. 굳이 10단어로 줄이려고 정보를 생략하지 말고, 다만 불필요하게 늘어지지 않게 대략 한 줄 분량(한국어 기준 30자 내외) 안에서 핵심만 담으세요.
- na: pass와 마찬가지로 짧게, 왜 해당 없는지만 간단히 적으세요.
전체 요약은 15단어 이내로 작성하세요.

3번(타이포·정렬·여백·명도대비)의 note는 위 규칙과 별도로 형식 하나가 더 있습니다 — 이 항목은 정렬·여백·명도대비·폰트(타이포) 4개 하위 영역이 한 항목에 묶여 있어서, 문제 하나만 적으면 나머지 영역을 실제로 확인했는지 결과만 봐서는 알 수 없습니다. 아래 형식으로 4개 영역을 빠짐없이 전부 언급하세요:
"정렬 OO / 여백·간격 OO / 명도대비 OO / 폰트 OO"
문제가 없는 영역은 "이상없음"으로 짧게, 문제가 있는 영역은 위 pass/needsfix 길이 기준에 맞게 필요한 만큼 적으세요. 예: "정렬 이상없음 / 여백·간격 카드 사이 불균일 / 명도대비 이상없음 / 폰트 2종 사용". 이렇게 하면 일부만 지적되고 나머지가 비어 보이는 대신, 4개 영역을 모두 실제로 확인했다는 게 결과에 드러납니다.
...
function formatContrastForPrompt(contrastFacts) {
  const lines = contrastFacts.map((f) => {
    ...
    const flag = Number.isFinite(f.ratio) && f.ratio < threshold ? ` — WCAG 기준(${threshold}:1) 미달` : '';
...
3번(타이포·정렬·여백·명도대비)의 텍스트-배경 대비를 판정할 때 이 수치를 참고하세요. 일반 텍스트는 4.5:1 미만이면 WCAG 기준상 가독성이 부족한 것으로 보고, [큰 텍스트로 추정] 표시가 붙은 항목은 3:1 미만일 때만 기준 미달로 보세요 — 헤드라인처럼 큰 텍스트는 WCAG상 원래 더 낮은 대비도 허용됩니다. 어느 쪽이든 기준선에 근소하게 못 미치는 수준(예: 일반 텍스트 3.5~4.5, 큰 텍스트 2.5~3 사이)이면 곧바로 needsfix로 올리지 말고 실제로 눈으로 봤을 때도 읽기 불편한지 시각적으로 한 번 더 확인해서 판단하세요. 명확히 기준 미달인 경우만 needsfix 이상으로 판정하세요.

단, 위 수치가 기준 미달로 나온 텍스트가 쿠폰 유효기간·사용조건·제외상품 안내, 법적고지·지급조건·면책사항 같은 부가조건·고지성 텍스트라면(텍스트 내용을 보고 판단) needsfix로 올리지 마세요 — 이런 텍스트는 핵심 카피보다 낮은 대비로 처리하는 게 정상적인 위계 표현입니다. 이 수치 기반 완화는 헤드라인·할인율·금액·CTA 같은 핵심 카피에는 적용하지 마세요.
