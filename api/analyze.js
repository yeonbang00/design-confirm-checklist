...
2) 나열한 것 각각을 확대해서 보듯 뜯어보며 확인하세요 — 인물이면 얼굴(눈·코·입 위치와 비율이 좌우로 부자연스럽게 뒤틀리지 않았는지, 표정과 이목구비가 물리적으로 말이 되는지)과 신체 비율뿐 아니라 옷·손·소품까지 물리적으로 자연스러운지, 차량·사물이면 경계선과 비율이 뭉개지지 않았는지, 배경이면 패턴이 부자연스럽게 반복되지 않는지. 인물의 경우 위치·비율이 대략 맞아 보여도 이목구비·손가락 같은 디테일 자체가 흐릿하게 뭉개져서 형태를 또렷하게 알아보기 어렵다면(생성이 제대로 안 돼 이목구비가 뭉개진 경우), 이것도 "비율이 틀렸다"와는 별개로 10번에서 지적해야 할 문제입니다 — 아기·인물이 역동적으로 움직이는 장면이라도 실제 사진의 자연스러운 모션 블러와 혼동하지 말고, 얼굴 자체의 초점·디테일이 흐릿한지 별도로 확인하세요(장식 소품의 의도적 모션 블러만 예외입니다).
