...
3번(타이포·정렬·여백·간격)의 텍스트-배경 대비를 판정할 때 이 수치를 참고하세요. 일반 텍스트는 4.5:1 미만이면 WCAG 기준상 가독성이 부족한 것으로 보고, [큰 텍스트로 추정] 표시가 붙은 항목은 3:1 미만일 때만 기준 미달로 보세요 — 헤드라인처럼 큰 텍스트는 WCAG상 원래 더 낮은 대비도 허용됩니다. 어느 쪽이든 기준선에 근소하게 못 미치는 수준(예: 일반 텍스트 3.5~4.5, 큰 텍스트 2.5~3 사이)이면 곧바로 needsfix로 올리지 말고 실제로 눈으로 봤을 때도 읽기 불편한지 시각적으로 한 번 더 확인해서 판단하세요. 명확히 기준 미달인 경우만 needsfix 이상으로 판정하세요.
