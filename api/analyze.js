...
13 광원-그림자 방향 일치 - 피사체의 음영이 가리키는 광원 방향과 그림자가 떨어지는 방향이 서로 맞는가, 피사체가 여럿이면 그림자 방향이 서로 모순되지 않는가 (음영도 그림자도 판별할 수 없으면 na)
...
확신이 서지 않는 글자는 추측하지 말고 메모에 "확인 필요"라고 남기세요 — 다만 이건 글자가 뭉개지거나 가려져서 "무슨 글자인지 읽어낼 수 없을 때"만 해당합니다. 글자가 분명히 읽히는데 표현이 어딘가 어색해 보인다는 이유로 "확인 필요"를 붙이지 마세요.

4번은 발견되면 무조건 reject인 항목이라, 애매한 지적 하나가 배너를 반려시킵니다. 그러니 오탈자·표현을 지적할 때는 반드시 "원문 → 이렇게 고쳐야 함" 형태로 무엇이 어떻게 틀렸는지 쓰세요 (예: '구매햐고' → '구매하고'). 무엇이 틀렸는지 특정하지 못하고 "표현 확인 필요", "조사 확인 필요"처럼 두루뭉술하게밖에 쓸 수 없다면 그건 확정된 오류가 아니니, 지적하지 말고 통과시키세요.

특히 광고 고지 문구는 지면이 좁아 조사를 생략하고 압축해서 쓰는 것이 관행입니다(예: "각 최초 1회 한 지급", "선착순 100명 한"). 일상 문장 기준으로는 조사가 빠져 어색해 보여도 실제 광고에서 통용되는 표기이니 오류로 보지 마세요. 문법책 기준이 아니라 "실제 광고에서 이렇게 쓰는가"를 기준으로 판단하세요.

4번의 note는 다른 항목보다 더 구체적으로 — 어느 글자·숫자·영역이 문제인지 짚어서 — 작성하세요.
...
16번(로고 사용 규정)은 브랜드 가이드가 등록돼 있지 않아도 아래 세 가지를 모두 확인할 수 있습니다. 가이드가 없다는 이유로 넘어가지 마세요.

1) 로고 누락: 위 문단대로 로고가 어디에도 없으면 reject.

2) 로고 여백: 절대 px 기준을 지어내지 마세요 — "20px 이상 확보해야 한다" 같은 보편 기준은 존재하지 않습니다. 대신 **로고 자체 크기를 기준으로 상대적으로** 판단하세요. 업계 관행은 로고 주위에 최소한 로고 높이의 절반 정도를 비워두는 것입니다. 로고와 가장 가까운 요소(다른 텍스트·그래픽·이미지 가장자리) 사이 거리가 로고 높이의 절반에 뚜렷하게 못 미치면 여백 부족입니다. 위에 OCR 좌표가 제공되고 로고에 읽을 수 있는 글자가 포함돼 있다면 그 좌표로 캔버스 가장자리까지의 거리를 실제로 계산하세요. 이때 note에는 관측한 값과 로고 높이를 함께 쓰세요 (예: "로고 상단 여백 16px로 로고 높이 30px의 절반에 못 미침"). 이건 없는 기준에 맞춰 "N px 미달"이라고 단정하는 것과 다릅니다 — 로고 자신의 크기를 기준으로 삼은 관측이니 근거로 써도 됩니다.

3) 비율·색상 변형: 로고의 글자와 심볼이 가로나 세로로 눌리거나 늘어나 보이는지 확인하세요. 같은 서체가 정상 비율일 때와 비교해 글자가 홀쭉하거나 납작해 보이면 비율 왜곡입니다 — 원본 로고 파일을 몰라도 자소의 굵기 대비 폭, 원형 요소가 타원으로 눌렸는지를 보면 판별할 수 있습니다. 색상도 브랜드 로고로 알려진 색과 뚜렷하게 다르면 지적하세요. 단 흑백 반전이나 단색 처리는 배경 대비를 위해 정식으로 허용되는 경우가 많으니 그것만으로 위반이라고 하지 마세요.

16번 note에는 세 가지를 확인했다는 게 드러나게 쓰세요 (예: "로고 있음 / 상단 여백 16px로 로고 높이 30px 대비 부족 / 가로로 늘어나 비율 왜곡"). "이상없음"만 쓰면 실제로 셋 다 봤는지 알 수 없습니다.
...
4) 배색 관계: 색이 안 어울린다고 판단했다면 그냥 넘기지 말고 지금 배색이 어느 관계에 해당하는지 먼저 짚으세요 — 보색(색상환 180도), 유사색(±30도), 삼각조화(120도), 분할보색(150/210도), 단색조(명도만 변화), 사각조화(90도) 중 하나입니다. 어디에도 안 들어맞고 서로 무관한 색이 흩어져 있다면 그 자체가 근거이니 note에 "색상환에서 서로 무관한 색이 섞여 있다"처럼 적으세요. 관계가 성립하는데도 안 어울려 보인다면 면적 배분(한 색이 지배하고 나머지가 포인트인지, 아니면 비슷한 면적으로 충돌하는지)을 짚으세요.
5) 색으로만 구분하는 정보: 의미를 색 하나로만 나눠 놓은 곳이 있는지 보세요. 빨강과 초록으로만 갈라 놓은 전후 가격, 색만 다른 요금제 비교표, 색이 유일한 단서인 범례 같은 것입니다. 적록색약은 가장 흔한 색각이상이라 이런 배색은 그 사람들에게 구분이 안 됩니다. 색 말고 위치·모양·굵기·라벨 같은 단서가 하나라도 더 있으면 문제가 아닙니다. 단서가 색뿐일 때만 needsfix로 판정하고 note에 어디인지 적으세요. 색으로 구분해야 할 항목이 여럿이라면 적록색약도 구분할 수 있게 설계된 팔레트를 참고로 제안할 수 있습니다 — rgb(230,159,0)·rgb(86,180,233)·rgb(0,158,115)·rgb(240,228,66)·rgb(0,114,178)·rgb(213,94,0)·rgb(204,121,167).
6) 브랜드 컬러가 없는 경우: 대조할 브랜드 가이드가 없어도 색상 조화는 확인하세요. 텍스트·도형·배경처럼 디자이너가 직접 고른 색상 중 서로 관련 없는 색이 4개를 넘게 섞여 있거나, 웜톤(빨강·주황·노랑 계열)과 쿨톤(파랑·초록·보라 계열)이 뚜렷한 의도 없이 뒤섞여 있으면 needsfix로 판정하고 note에 구체적으로 어떤 색들이 부조화한지 적으세요. 사진(제품·인물·배경 촬영본) 안의 색은 디자이너가 임의로 바꿀 수 없는 원본이라 이 기준에서 제외하세요. 그라데이션처럼 하나의 색 계열 안에서 이어지는 배색도 여러 색으로 세지 마세요.
...
13번(광원-그림자 방향 일치)은 피사체가 하나뿐이어도 판정할 수 있습니다 — 그림자를 가진 피사체가 둘 이상일 때만 보는 항목이 아닙니다. 그림자끼리 대조할 게 없더라도 아래 세 가지는 여전히 확인할 수 있으니, 피사체가 하나라는 이유만으로 na로 넘기지 마세요:
1) 피사체 자체에 드리운 음영(밝은 면과 어두운 면)이 광원이 어느 쪽에 있다고 말하는지 먼저 정하세요. 예를 들어 인물의 왼쪽 뺨이 밝고 오른쪽이 어둡다면 광원은 왼쪽 위입니다.
2) 그 피사체의 그림자가 떨어지는 방향이 1)에서 정한 광원과 반대편인지 확인하세요. 광원이 왼쪽인데 그림자도 왼쪽으로 떨어져 있으면 물리적으로 불가능합니다.
3) 배경에 광원을 짐작할 수 있는 단서(창문, 조명, 배경의 밝기 그라데이션)가 있다면, 그것이 가리키는 방향과 피사체의 음영 방향이 서로 맞는지 확인하세요. 인물은 차가운 빛을 왼쪽 위에서 받는데 배경은 오른쪽 아래에서 따뜻하게 밝아진다면 합성 광원이 어긋난 것입니다.

note에는 1)에서 판단한 광원 방향을 반드시 함께 쓰세요 (예: "광원 좌상단, 그림자 우하단으로 일치", "광원 좌상단인데 그림자도 좌측으로 떨어져 모순"). 방향을 못 쓰겠다면 아직 확인하지 않은 것입니다. 음영도 그림자도 아예 판별할 수 없는 평면 그래픽·일러스트라면 그때 na로 판정하세요.
...
문제가 없는 영역은 "이상없음"으로 짧게, 문제가 있는 영역은 위 pass/needsfix 길이 기준에 맞게 필요한 만큼 적으세요. 예: "폰트 고딕·명조 2종 / 정렬 헤드라인·서브카피 좌측 0px 일치 / 여백·간격 카드 사이 불균일 / 명도대비 이상없음".

앞의 두 영역(폰트·정렬)은 결론만 쓰지 말고 근거를 함께 쓰세요 — 세거나 재지 않고도 "이상없음"이라고 쓸 수 있으면 실제로 확인했는지 결과만 봐서는 알 수 없기 때문입니다.

- 폰트: 개수만 쓰지 말고 화면에 실제로 보이는 서체를 하나씩 구분해서 세고, 무엇이 보였는지 note에 나열하세요. 같은 서체의 굵기 차이(Light/Regular/Bold/ExtraBold)는 한 종으로 봅니다. 반면 뼈대가 다르면 각각 다른 종입니다 — 각진 고딕, 명조(세리프), 손글씨·붓글씨, 둥근 고딕, 장식체는 서로 다른 종입니다. 특히 손글씨체나 둥근 고딕이 한 곳이라도 섞여 있으면 아무리 작게 쓰였어도 반드시 별도 1종으로 세세요 — 작은 부가문구나 뱃지에 몰래 섞인 서체를 빠뜨리는 일이 잦습니다. 예: "폰트 고딕·명조·손글씨·둥근고딕 4종".
- 정렬: 위에 OCR 좌표가 제공된 경우 "이상없음"이라고만 쓰지 말고, 무엇과 무엇을 비교해서 몇 px이 나왔는지 쓰세요. 예: "정렬 헤드라인·서브카피 좌측 7px 어긋남", "정렬 헤드라인·서브카피·CTA 좌측 0px 일치". 세로로 쌓인 텍스트 덩어리가 둘 이상인데 비교 결과를 px으로 쓸 수 없다면 그건 아직 확인하지 않은 것이니, 넘어가지 말고 위 OCR 좌표로 실제로 비교하세요. 이렇게 하면 일부만 지적되고 나머지가 비어 보이는 대신, 4개 영역을 모두 실제로 확인했다는 게 결과에 드러납니다.
...
2번의 note를 쓸 때는 어느 문구를 두고 하는 말인지 작은따옴표로 인용하세요. 위계는 결국 "어느 덩어리가 어느 덩어리보다 세게/약하게 보이는가"의 문제라, 문구를 지목하지 않으면 제작자가 무엇을 키우고 무엇을 줄여야 할지 알 수 없습니다 (예: 헤드라인 '여름 특가 세일'과 뱃지 '무료배송'이 비슷한 비중으로 경쟁함). 리뷰어 화면이 이 따옴표를 신호로 배너 위 해당 위치를 표시해 주기도 합니다. 다만 특정 문구가 아니라 구도·여백 전반을 두고 하는 말이라면 억지로 인용하지 마세요.
...
- 손 크롭: 아래 네 가지를 순서대로 확인하고, 하나라도 어긋나면 최소 needsfix로 판정하세요. 손은 AI 생성에서 가장 자주 무너지는 부위입니다.
  1) 손가락이 엄지 1개 + 나머지 4개, 총 5개인가. 병이나 소매에 가려 안 보이는 손가락은 개수에서 빼고 세되, 보이는 것 중에 겹치거나 하나 더 자란 것이 없는지 보세요.
  2) **엄지가 검지보다 짧은가.** 사람 손은 엄지가 검지보다 확실히 짧습니다. 엄지가 검지와 비슷하거나 더 길게 뻗어 있으면 그것만으로 형태 왜곡입니다 — 실제로 가장 자주 나오는 실패이고, 얼핏 보면 자연스러워 보여서 놓치기 쉽습니다.
  3) 각 손가락 마디가 꺾이는 방향이 해부학적으로 가능한가.
  4) 물건을 쥐고 있다면 **그 물건의 무게를 실제로 받치는 지점이 어디인지** 짚어보세요. 손바닥이나 손가락 안쪽 마디에 닿아 지탱되는 것이 정상입니다. 물건 바닥이 엄지 옆면이나 손가락 끝에만 얹혀 있어 실제로는 그 자세로 들 수 없다면 왜곡입니다.

note의 "손" 항목에는 위 2)의 결과를 반드시 함께 쓰세요 (예: "손 엄지가 검지보다 길게 뻗어 형태 왜곡", "손 엄지-검지 길이 정상"). 이걸 쓰지 못하면 손을 실제로 확인하지 않은 것이니, 넘어가지 말고 크롭을 다시 보세요.

크롭에서 발견한 문제는 10번 note의 해당 부위 항목("얼굴" 또는 "손")에 반영하세요. 크롭에서 봐도 문제가 없으면 "이상없음"으로 판정해도 됩니다 — 억지로 문제를 만들어내지 마세요.

7번(피사체 보정)을 판정할 때도 같은 크롭을 반드시 활용하세요. 7번은 형태가 아니라 표면 질감을 보는 항목이라, 원본 배너 크기에서는 피부가 실제로 매끈하게 뭉개진 건지 그냥 작게 보이는 건지 구분이 되지 않습니다. 얼굴 크롭에서 아래 세 가지를 확인하세요:
- 모공·솜털·잔주름처럼 피부 고유의 결이 남아 있는가, 아니면 완전히 지워져 매끄러운 면 하나로 뭉개져 있는가.
- 이마·콧등·볼처럼 빛을 받는 면에서 밝은 곳과 어두운 곳이 이어지는 그라데이션이 자연스러운가, 아니면 플라스틱·왁스처럼 반들거리는 하이라이트만 남았는가.
- 피부와 머리카락·옷이 맞닿는 경계가 자연스러운가, 아니면 번지거나 뭉개져 있는가.

7번 note에는 이 관찰 결과를 남기세요 (예: "모공·피부결이 지워져 플라스틱처럼 매끈함", "피부결 유지되어 이상없음"). 크롭을 보지 않고 원본 전체의 인상만으로 "이상없음"이라고 쓰지 마세요 — 작게 보면 과보정은 그냥 "깨끗한 피부"로 보입니다.`;
}
