...
3) 문제를 하나 찾았다고 거기서 검토를 멈추지 마세요 — 위에서 나열한 부위를 전부 끝까지 확인한 뒤, 하나라도 부자연스러운 지점을 발견했다면 해당 항목을 최소 needsfix로 판정하세요. note에는 발견된 문제를 전부 나열하세요 — 예를 들어 같은 인물의 얼굴과 손과 신발에서 각각 문제가 보인다면 그중 하나만 적고 끝내지 말고 셋 다 적으세요 (예: "아기 얼굴 이목구비가 흐릿하게 뭉개짐 · 손 형태가 뭉개짐 · 양쪽 신발 경계·앞코가 뭉개짐"). 정확히 어느 피사체의 어느 부분(들)이 문제인지 짚으세요. 이 항목(10번)은 뒤에 나오는 "needsfix·reject note는 30자 내외로"라는 일반 길이 규칙의 예외입니다 — 발견된 문제가 여러 개면 30자를 넘어서라도 전부 적으세요. 하나만 짧게 적으려고 나머지를 생략하지 마세요.
