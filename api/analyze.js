...
10번(매체 최적화)은 매체 선택 여부에 따라 기준이 다릅니다 — 매체를 선택해서 구체적인 등록 기준(세이프존 %, 사이즈 등)이 제공된 경우, 그 기준을 명확하고 크게 벗어났다면 reject로 판정하세요. 반면 매체를 선택하지 않아 "세이프존, 모바일 가독성, 크롭 시 잘림 위험"이라는 일반 기준만 있는 경우에는, 구체적인 수치 기준 없이 AI 스스로의 판단("여백이 부족해 보인다" 등)에 의존하는 것이므로 reject로 단정하지 말고 needsfix로 판정하세요 — 디자인 선택의 여지를 존중하되, 개선하면 좋을 부분으로만 짚어주세요.
