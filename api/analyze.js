...
// Kept as ONE list so every item shares the same pass/needsfix/reject/na scale.
...
10 매체 최적화 - 세이프존, 모바일 가독성 (일부 매체는 기기별 화면비율에 맞춰 자동으로 크롭될 수 있음 — 해당 매체 기준이 제공된 경우에만 확인)
11 최종 디테일 - 요소 간 여백·간격의 일관성, 텍스트 줄바꿈 위치의 자연스러움
12 신체 비율 왜곡 - AI 생성/보정 특유의 신체·의상·소품 형태 왜곡 (인물이 없으면 na)
...
18 로고 사용 규정 - 로고 최소 여백 확보, 변형·왜곡 없이 사용, 로고 자체가 누락되지 않았는지
19 법적고지·심의 문구 - 금융/대출/의료/주류/게임 등 규제 업종에 필요한 고지·심의 문구 포함 여부 (해당 업종이 아니면 na)

19개 항목은 pass/needsfix/reject/na 중 하나로 판정하세요.
- pass: 문제 없음
...
- reject: 광고주 전달 전 무조건 확인·수정해야 하는 문제입니다. "시안을 통째로 다시 만들어야 할 정도"라는 뜻이 아니라, 고치는 것 자체는 쉽더라도 반드시 짚고 넘어가야 하는 항목이라는 뜻입니다. 아래 두 기준 중 하나에 해당하면 reject로 판정하세요:
  1) 3번(정렬 관련 부분)·4번·11번·18번에서 발견된 문제는 크기와 무관하게 무조건 reject로 판정하세요(needsfix로 낮추지 마세요) — 오탈자, 여백·정렬, 로고 사용 규정처럼 객관적으로 확인 가능한 필수 준수 사항이기 때문입니다.
  2) 그 외 항목(1,2,5,6,7,8,9,12,13,14,15,16,17)은 문제가 대부분의 사람이 봐도 명백하게 눈에 띄는 저퀄리티(AI 생성 아티팩트가 뚜렷하거나 합성이 확연히 부자연스러운 경우 등)일 때만 reject로 판정하고, 미묘하거나 선택적으로 더 다듬으면 좋은 수준이라면 needsfix로 판정하세요.
10번(매체 최적화)은 매체 선택 여부에 따라 기준이 다릅니다 — 매체를 선택해서 구체적인 등록 기준(세이프존 %, 사이즈 등)이 제공된 경우, 그 기준을 명확하고 크게 벗어났다면 reject로 판정하세요. 반면 매체를 선택하지 않아 "세이프존, 모바일 가독성"이라는 일반 기준만 있는 경우에는, 구체적인 수치 기준 없이 AI 스스로의 판단("여백이 부족해 보인다" 등)에 의존하는 것이므로 reject로 단정하지 말고 needsfix로 판정하세요 — 디자인 선택의 여지를 존중하되, 개선하면 좋을 부분으로만 짚어주세요.
...
19번(법적고지·심의 문구)은 예외입니다 — 이미지만 보고 어떤 문구가 정확히 필요한지, 표기가 규정에 맞는지까지는 확신할 수 없으므로 reject로 단정하지 마세요. 고지 문구가 아예 안 보이거나 너무 작아 알아보기 어려운 경우 needsfix로 판정하고, note에 "정확한 필요 문구·표기 기준은 담당자 확인 필요"라고 남기세요. 문구가 있고 육안으로 특별히 이상해 보이지 않으면 pass로 판정하세요.
- na: 해당 시안에 명백히 적용되지 않는 항목 (예: 인물 없는 시안의 12번, 비규제 업종의 19번)
...
4번(정보 정확성), 10번(매체 최적화), 11번(최종 디테일)은 팀이 가장 중요하게 보는 기준입니다. 4번을 판정하기 전에, 다른 항목처럼 인상만 보고 넘어가지 말고 반드시 아래 절차를 실행하세요:
...
11번(최종 디테일)을 판정할 때는 오탈자·정렬이 아니라 다음 두 가지만 확인하세요:
...
금액 표기 띄어쓰기: "3만원대", "3만원 대"는 팀 표기 컨벤션 위반입니다. "숫자+공백+원+대"(예: "3만 원대", "10만 원대")가 올바른 표기이니, 이미지 속 금액 표기가 이 형식과 다르면 4번에서 지적하세요.
...
3번(타이포·정렬)을 판정할 때, 텍스트 안에 가운데점(·)·하이픈(-)·별표(*) 같은 구분 기호가 쓰였다면 확대해서 보듯 자세히 확인하세요: 그 기호가 양쪽 글자 사이에 수직으로 정확히 가운데 위치하는지, 기호 양옆 여백이 대칭적으로 자연스러운지를 보세요 (예: "코인·배지"에서 점이 위아래 중앙에 있고, 점과 "코인" 사이 간격과 점과 "배지" 사이 간격이 비슷한지). 기호가 한쪽으로 치우쳐 있거나 양옆 여백이 눈에 띄게 다르면 3번을 reject로 판정하고, note에 어느 구분 기호가 어떻게 치우쳤는지 구체적으로 적으세요.

2번(위계 및 구조)·3번(타이포·정렬)을 판정할 때 예외: "(특약)" 같은 부가조건 표기, 법적고지·심의 문구, 지급조건·면책사항 안내 같은 필수 고지성 텍스트는 메인 카피와 다른 기준으로 보세요. 이런 텍스트는 원래 크게 강조하지 않고 작게, 카피 끝이나 화면 구석에 붙여서 시각적으로 눈에 덜 띄게 처리하는 게 업계 관행입니다 — 광고주가 꼭 넣어야 해서 넣는 것이지 강조하고 싶어서 넣는 게 아니기 때문입니다. 그러니 이런 텍스트가 메인 카피처럼 가운데 정렬되어 있지 않거나, 헤드라인 시선 동선 밖에 작게 붙어 있다고 해서 정렬 오류나 위계 문제로 판정하지 마세요. 완전히 안 보이거나 잘려나가지 않는 이상, 작게·구석에·낮은 비중으로 배치된 것 자체는 정상입니다.

15번(워터마크·서명 흔적)을 판정할 때 중요한 예외: "AI로 생성된 이미지입니다", "AI로 생성된 제작물", "Made with AI", "AI-generated", "* AI 생성" 등 AI 생성 사실을 알리는 고지 문구(한글·영문 표현 모두 포함)는 15번이 잡으려는 "결함"이 아닙니다 — 오히려 의도적으로 넣은 필수 고지 문구입니다. 이런 문구가 이미지 구석에 작게 적혀 있다고 해서 15번을 reject나 needsfix로 판정하지 마세요. 15번은 원래 목적대로, 광고주가 의도하지 않은 스톡 이미지 사이트의 워터마크(예: Shutterstock, Getty Images 로고)나 생성 도구가 실수로 남긴 서명·로고(예: 의도치 않게 찍힌 "Midjourney" 같은 텍스트)가 남아있는지만 확인하세요.

18번(로고 사용 규정)을 판정할 때, 이미지 전체를 꼼꼼히 살펴 브랜드 로고나 워드마크가 어디에도 없다면 na로 넘어가지 말고 reject로 판정하세요 — note에 "로고가 보이지 않음 — 브랜드 로고 누락 여부 확인 필요"라고 남기세요. 로고가 있는데 여백·변형 등에 문제가 없다면 pass로 판정하세요.

5번(컬러 일관성)을 판정할 때는 다음을 확인하세요:
...
2) 텍스트-배경 대비(가독성): 텍스트 색상과 배경 색상이 비슷한 톤이라 눈으로 봐도 명백하게 글자가 묻혀 보이는 경우(예: 어두운 파란 배경 위 검정 텍스트, 밝은 배경 위 연한 회색 텍스트)를 확인하세요. 대비가 근소하게 낮은 수준까지 잡을 필요는 없고, 실제로 읽기 불편할 정도로 명백한 경우만 needsfix 이상으로 판정하고 note에 "텍스트-배경 대비 부족: OO 텍스트가 OO 배경에 묻힘"이라고 적으세요.
3) 브랜드 컬러 예외: 브랜드 컬러 가이드가 제공된 경우 원칙적으로 그 기준을 따르되, 이미지에서 여름(바다·해변 등 파란 계열), 크리스마스·연말(레드·그린·골드 계열)처럼 명확한 시즌·테마 컨셉이 드러난다면 브랜드 컬러와 다른 계열을 쓴 것을 미준수로 단정하지 말고, 시즌 컨셉과 잘 어울리는 배색인지로 판단하세요.
4) 색상 조합 제안: 지금 배색이 눈에 잘 띄지 않거나 개선 여지가 있다고 판단되면, 아래 참고 배색 중 배너 성격(퍼포먼스/브랜딩)과 톤에 맞는 조합을 구체적으로 언급하며 제안하세요. 다만 "이렇게 바꾸세요"처럼 단정하지 말고 "이런 계열도 고려해볼 만하다"는 참고용 제안으로 표현하세요 — 최종 색상 선택은 디자이너의 몫입니다. 브랜드 컬러가 지정된 시안이라면 그 브랜드 컬러 범위 안에서만 제안하세요.

[참고 배색 — 컬러 가이드 페이지에 정리된 조합]
- 눈에 잘 띄는 조합: 옐로우×블랙(#FFD400·#111111), 레드×화이트(#E63946·#FFFFFF), 네온그린×블랙(#B4FF39·#0D0D0D), 핫핑크×퍼플(#FF3EA5·#7B2FF7), 오렌지×딥블루(#FF7A00·#0B2545)
- 시즌·업종별: 여름(#0077B6·#FFD60A), 크리스마스·연말(#B3001B·#1B4332·#C9A227), 뷰티·코스메틱(#F7CAD0·#C8B6FF), 금융·보험(#0A2647·#205295), 푸드·배달(#D62828·#F77F00)
- 그라데이션: 선셋(#FF7A00→#FF3EA5→#7B2FF7), 블루(#0B2545→#1E6091), 퍼플-핑크(#7B2FF7→#FF3EA5), 그린-틸(#004E64→#00A896)

13번(텍스트·로고 렌더링)을 판정하기 전에, 반드시 아래 절차를 순서대로 실행하세요:
...
2) 앱 아이콘 확인(정사각형 둥근 모서리 앱 아이콘이 있는 배너에만 해당): 이미지 안에 정사각형(둥근 모서리) 앱 아이콘이 있다면 몇 개인지, 각각 어떤 서비스인지 나열하세요. 앱 아이콘이 하나도 없다면 이 단계는 그냥 건너뛰고, "앱 아이콘 없음" 같은 문구를 note에 남기지 마세요 — 앱 아이콘 유무는 13번 판정과 무관한 정보입니다.
...
5) 배경색이 위 정답과 명확히 다르면(예: 넷플릭스가 빨간 배경, 디즈니+가 순수 블루나 보라색 등) 13번을 최소 수정필요로 판정하세요. 오류가 발견된 아이콘이 여러 개면 메모에 전부 짧게 나열하세요 (예: "넷플릭스·디즈니+ 배경색 오류"). 하나만 골라서 적지 마세요.
5) 목록에 없는 서비스 아이콘은 추측해서 판정하지 말고 넘어가세요.

12·14·16·17번(AI 생성 아티팩트)을 판정하기 전에도 인상만 보고 넘어가지 말고 아래 절차를 실행하세요:
1) 이미지 안에 있는 사람·동물·차량·제품 등 개별 피사체와, 물방울·연기·빛번짐 같은 질감 효과를 모두 나열하세요.
2) 나열한 것 각각을 확대해서 보듯 뜯어보며 확인하세요 — 인물이면 신체 비율뿐 아니라 옷·손·소품까지 물리적으로 자연스러운지, 차량·사물이면 경계선과 비율이 뭉개지지 않았는지, 배경이면 패턴이 부자연스럽게 반복되지 않는지.
...
- 퍼포먼스: 할인·혜택·기간 한정, 특정 제품 판매, 행사·캠페인 참여 유도 등 구매·전환 유도가 핵심 (프로모션·커머스·이벤트 성격의 배너를 모두 포함)
- 브랜딩: 직접적 판매 목적 없이 브랜드 인지도·이미지 전달이 핵심
- 기타: 위에 해당하지 않음
bannerType 필드에 이 중 하나를 한국어로 응답하세요.

유형별 평가 기준 조정: 배너 유형이 "브랜딩"으로 판단되면, CTA나 구매 유도 문구가 없다는 이유만으로 2번(위계 및 구조)과 9번(CTA 및 전환)을 needsfix나 reject로 판정하지 마세요 — 브랜딩 배너는 CTA 부재가 의도된 디자인입니다. 대신 브랜드 메시지 전달력과 톤 일관성을 기준으로 평가하세요. "퍼포먼스" 유형은 기존 기준대로 CTA와 혜택 명확성을 엄격하게 평가하세요.
...
2) 나열한 메시지들이 하나의 핵심 메시지로 수렴하는지, 아니면 서로 다른 방향의 메시지가 2개 이상 동시에 강조되고 있는지 확인하세요 (예: "반값 할인"과 "신제품 출시"와 "이벤트 참여"가 한 배너에 동시에 크게 들어간 경우).
3) 위에서 판단한 bannerType에 맞는 톤인지 확인하세요 — 예를 들어 브랜딩인데 할인 문구가 과하게 강조되어 있거나, 프로모션인데 정작 핵심 혜택이 잘 안 보이는 경우.
4) 메시지가 2개 이상으로 분산되어 있거나 유형에 안 맞는 톤이면 1번을 최소 needsfix로 판정하고, note에 어떤 메시지들이 충돌하는지 구체적으로 적으세요. 메시지가 하나로 명확히 수렴하고 톤도 유형에 맞을 때만 pass를 주세요.`;
...
export default async function handler(req, res) {
  ...
  try {
    const parsed = await callOpenAI({
      ...
      // 오탈자 확인(4번)이 팀 최우선 기준인데, effort가 낮으면 19개
