...
const AXES = {
  ...
  model: [
    ['A', '제품 중심', '인물은 두되 배경을 걷어내고, 착용한 제품이 가장 잘 보이는 각도와 자세로 다시 잡는다.'],
    ['B', '사용 장면', '인물이 이 옷을 입고 실제로 무언가 하고 있는 장면. 그 행동에 맞는 자세와 구도로 새로 잡는다.'],
    ['C', '분위기',   '색과 빛으로 인상을 만든다. 그 빛에 어울리는 자세와 시선으로 다시 잡는다.'],
  ],
};
...
2. 제품 자체는 바꾸지 않는다. 형태·색·포장·개수는 그대로다. 바꾸는 것은
   ...
   장면에 맞는 앵글로 다시 잡아라.
${kind === 'model' ? `
2-1. 원본은 사람이 착용한 사진이다. 세 장 모두 그 인물을 유지한다.
   유지할 것 — 같은 사람의 얼굴·머리·체형, 착장(제품)의 형태·색·디테일.
   바꿔도 되는 것 — 자세, 팔다리 위치, 시선, 카메라 각도와 거리, 배경.

   자세를 원본 그대로 복사하지 마라. 같은 사람이 그 장면에서 자연스럽게
   취할 자세로 다시 잡아라. 서점 앞이면 걷거나 무언가 보고 있고, 밤거리면
   빛을 향해 서 있는 식이다. 배경만 갈아끼운 합성처럼 보이면 실패다.
   인물을 지우고 옷만 남기지도 마라.` : ''}
