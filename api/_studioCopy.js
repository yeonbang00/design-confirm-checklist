...
import {chooseCopyFacts,completeCopy,validatePlannedCopy} from '../assets/studio-evidence.mjs';
...
export async function studioCopy(req,res,apiKey) {
  try {
    ...
    const shots=Array.isArray(req.body.plans)?req.body.plans.slice(0,6).map(x=>{
      const a=x&&x.axes||{};
      return {기본문구:x?.copyMode==='basic'?basicCopy(product):null,무드:x?.visualTone||null,레퍼런스배치:x?.designObservation||null,디자인방향:String(x?.artDirection||'').slice(0,700),제작의도:String(x?.copyBrief||'').slice(0,300),
...
같은 제목 두 줄을 의미 없이 다른 크기로 강조하지 않는다. 로고나 추가 브랜드 라벨을 만들지 않는다.
    ...
    for(let attempt=0;attempt<2;attempt++){
      ...
      try{
        ...
        copies=result.copies.map((row,i)=>{
          ...
          const copy=plan.copyMode==='basic'?basicCopy(product):validatePlannedCopy(resolveCopy(row,product,facts),plan,product);
