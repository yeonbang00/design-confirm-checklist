...
- axes: 이 배너를 아래 ${AXIS_KEYS.length}개 축으로 설명하세요. **각 축마다 반드시 목록 안의 영문 id 하나만** 쓰세요.
${axisBlock}

  축을 고르는 기준입니다.
  · frame은 화면 분할 방식만, focus는 가장 크게 보이는 주인공만 고릅니다. 인물 전면을 프레임으로 분류하지 마세요.
  · motif는 배치를 만드는 가장 두드러진 장치를 고릅니다. 거대한 달·원은 moon, 제품을 비추는 빛줄기는 spotlight, 저울은 balance입니다. 색이나 업종이 달라도 같은 장치는 같은 값입니다. 없으면 none, 목록 밖의 명확한 장치는 other입니다.
  · layout은 카피와 그림의 상대 위치입니다. 상단 헤드라인과 하단 CTA가 공통이면 가운데 제품이 달라도 그 구조를 일관되게 읽으세요.
