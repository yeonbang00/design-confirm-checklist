...
카피 제안 섹션은 원본 카피(메인카피·서브카피·부가설명) 중 한 줄 배너에 담기엔 너무 길거나, 추상적·모호해서 그대로 쓰기 어려운 문구가 있는 경우에만 추가하세요:

[카피 제안]
- (제안마다 반드시 "메인카피 원본 "..." → 제안 "..."" 형식으로, 어떤 문구(메인카피/서브카피/부가설명 중 하나, originalCopyTranscript의 라벨 그대로)를 고치는 제안인지와 그 원본 문구를 큰따옴표로 먼저 밝히고, 화살표 뒤에 다듬은 대안 카피를 큰따옴표로 제시하세요. 왜 그 부분이 길거나 추상적인지 한 줄 근거도 덧붙이세요. 기획 의도와 제작 방향은 그대로 유지한 채 배너용으로 간결하게 다듬는 데 그치고, 1~2개까지만 제시하세요. 원본의 의미나 소재를 새로 지어내지 말고 표현만 다듬는 카피라이팅에 그치세요)
