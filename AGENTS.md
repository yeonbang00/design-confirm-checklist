...
**조판을 고쳤으면 `layout-audit.html`을 열어라.** 조판 열여덟이 항목가이드 2번
(위계 및 구조)을 지키는지 숫자로 잰다 — 위계 1.25배, 덩어리 4개 미만, CTA 크기,
간격 일관성. 처음 쟀을 때 18/18 위반이었다. 지금은 0/18이고, 그 상태를 지켜야 한다.
이 파일은 banner-studio.html의 인라인 `<style>`을 복사해 두므로 그쪽이 바뀌면
다시 만들어야 한다.

**화질이 속도를 거의 전부 결정한다.** 실측 — 같은 프롬프트로 gpt-image-2 1024
edits가 low 17.2초 / medium 38.9초 / high 108.5초. 기본은 medium이고, 고른 한 장만
카드에서 high로 다시 뽑는다. 여섯 장을 다 high로 뽑으면 생성만 3분이 넘는다.
