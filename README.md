...
11개 디자인 기본기 + 6개 AI 생성 아티팩트 + 2개 광고주 전달 전 필수 확인(로고 사용 규정, 법적고지·심의 문구) — 총 19개 항목을 하나의 목록으로 판정합니다. 각 항목은 통과 / 경미수정 / 주요수정 / 반려 / 해당없음 중 하나로 표시됩니다.

## 광고주별 성과 배너 비교 기능 (집중 비교)

체크리스트 판정과 별개로, "이 광고주는 예전에 이런 배너가 성과가 좋았다"는 기준 이미지를 등록해두면,
새 시안을 올릴 때 그 기준 배너들과 비교해서 **닮은 점 / 부족한 점**을 같이 보여줍니다.

- 화면의 "비교 기준 광고주" 드롭다운에서 광고주를 고르고 이미지를 분석하면 자동으로 비교됩니다.
- 광고주를 고르지 않으면(기본값 "비교 없이 분석") 기존처럼 체크리스트만 판정합니다.

**광고주/기준 배너를 추가하려면**: 기준 배너 이미지와 광고주 이름을 Claude와의 채팅에 올리고
"이 광고주 기준 배너로 추가해줘"라고 요청하세요. Claude가 `api/_referenceBanners.js` 파일의
업데이트된 코드를 드리면, GitHub에서 그 파일을 열어 내용을 교체하고 Commit하면 됩니다
(지금까지 `analyze.js`를 고쳤던 것과 동일한 방식입니다). 코드에 base64로 직접 저장되는 방식이라
별도의 데이터베이스나 이미지 호스팅이 필요 없습니다.

## 파일 구조

```
design-confirm-checklist/
├─ index.html               # 프론트엔드 (체크리스트 UI + 업로드 + 광고주 선택)
├─ api/
│  ├─ analyze.js             # 서버 함수 (Gemini API 호출, 키 보관, 비교 로직)
│  ├─ advertisers.js         # 광고주 목록 조회 API
│  └─ _referenceBanners.js   # 광고주별 기준 배너 데이터
├─ package.json
├─ .env.example
└─ README.md
```
