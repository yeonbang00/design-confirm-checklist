...
| 페이지 | 설명 |
|---|---|
| `index.html` | 디자인 체크리스트 — 배너 시안을 올리면 19개 항목을 AI가 자동 판정 |
...
이미지 분석용 API 키는 절대 브라우저(프론트엔드)에 있으면 안 됩니다. 그래서 각 페이지는 우리 서버의 `/api/*`만 호출하고, 그 서버 함수 안에서만 Gemini API를 진짜 키로 호출하는 구조입니다. 키는 배포 플랫폼의 "환경변수"에만 저장되고, 코드나 브라우저 어디에도 노출되지 않습니다.
...
| 이름 | 용도 |
|---|---|
| `GEMINI_API_KEY` | 배너/기획안 분석용 Gemini API 키 |
...
11개 디자인 기본기 + 6개 AI 생성 아티팩트 + 2개 광고주 전달 전 필수 확인(로고 사용 규정, 법적고지·심의 문구) — 총 19개 항목을 하나의 목록으로 판정합니다. 각 항목은 **통과 / 수정필요 / 반려 / 해당없음** 중 하나로 표시됩니다.
...
- **"서버에 GEMINI_API_KEY 환경변수가 설정되어 있지 않습니다"** → Vercel 환경변수 설정을 확인하고 재배포하세요.
