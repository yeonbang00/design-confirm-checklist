...
function art(v){const photos=v.photos||PHOTOS,
 ...
 p=photos[pi]||photos[0],pool=(v.mode||'original')==='original'?photos.filter(x=>x.kind==='original'):photos,second=(Array.isArray(v.photoSet)&&photos[v.photoSet[1]])||pool[(pool.indexOf(p)+1)%pool.length]||p;const cut=p&&p.cutUrl&&['offer','type-diagonal','arch','framed','numeral'].includes(v.layout);
...
function imageFileName(v,i){
  ...
  const tag=photo.kind==='ai'?'AI':photo.sourceRegion?'상세-추출':(photo.cleanUrl?'원본-로고제거':'원본');
  const scene=(photo.label||'').split('·')[0].trim().replace(/\s+/g,'') || ('시안'+(i+1));
  const ext=photo.cleanUrl?(photo.cleanType==='image/jpeg'?'jpg':'png'):(String(photo.url||'').split('?')[0].match(/\.(png|jpe?g|webp)$/i)||[,'png'])[1];
...
async function downloadImage(i){
  ...
  if(v.baked&&!PHOTOS[v.photo]?.baseImageUrl){const details=$$('[data-card]')[i]?.closest('.card')?.querySelector('details');if(details){details.open=true;details.scrollIntoView({block:'center',behavior:'smooth'})}notify('광고 글자 제거 실행 후 제거본을 다운로드하세요. API 비용이 발생합니다.');return;}
...
async function downloadAllImages(){
  if(variants.some(v=>v.completeBanner)){
   for(const [i,v] of variants.entries())if(v.imageReady!==false&&!v.imageFailed){
    try{const el=$$('[data-card]')[i]?.querySelector('.art');if(el)saveBlob(await exportBanner(el,getJSON),imageFileName(v,i).replace(/\.[^.]+$/,'_완성.png'));}catch(e){notify('배너 저장 실패: '+e.message);return;}
   }
   notify('완성 배너 다운로드를 요청했습니다.');return;
  }
...
function renderSaved(){
 if($('#savedCount'))$('#savedCount').textContent=saved.length+recipes.length;$('#emptySaved').hidden=!!(saved.length+recipes.length);$('#exportAll').disabled=!(saved.length+recipes.length);$('#variantSavedTitle').hidden=!saved.length;$('#recipeSavedTitle').hidden=!recipes.length;
...
const GRAB_BEST=5;      // 후기·추천상품을 걸러내는 점수 규칙이 들어간 판
...
async function autoCopies(run){
 const refs=[];if(run!==autoRun)return;
 const references=autoPlan.map(p=>({...p.reference,type:p.type,typeLabel:REF_TYPES[p.type],designObservation:p.designObservation||null}));
 const {photos:photoBytes,sourcePhotos:sourceBytes,...copyProduct}=product;
...
async function classifyPhotos(run){
 ...
 product.photos=await tileDetailPhotos(product.photos,getJSON,()=>run===autoRun);
 if(run!==autoRun)return;
 PHOTOS.splice(0,PHOTOS.length,...product.photos);
 ...
 try{
  ...
  for(let offset=1;offset<urls.length||offset===1;offset+=5){
   ...
   batches.push(await getJSON('/api/productPhotos',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({urls:batch,tiles:product.photos.filter(p=>batch.includes(p.url)&&p.detailTile).map(p=>({url:p.url,base64:p.cleanBase64,mediaType:p.cleanType})),referenceUrls:offset===1?referenceUrls:[],productName:product.productName,brand:product.brand,sourceUrl:product.sourceUrl,pageSections:offset===1?evidenceSections(product):[]})}));
   if(run!==autoRun)return;
  }
  const data={...batches[0],photos:[...new Map(batches.flatMap(b=>b.photos||[]).map(p=>[p.url,p])).values()],facts:[...new Map(batches.flatMap((b,batch)=>(b.facts||[]).map((f,i)=>({...f,id:'image-batch-'+batch+'-'+i}))).map(f=>[f.text,f])).values()]};
  if(run!==autoRun)return;
  ...
  const usable=product.photos.filter(p=>(p.role!=='unusable'||p.regions?.length)&&p.matchesTarget===true);
...
async function readPalette(run){
 const main=product.photos[0];if(!main||main.sourceRegion)return;
...
async function scrubLogos(run){
 ...
 const results=await Promise.all(targets.map(async p=>{
  try{const out=await scrubLogo(p.url,getJSON);
   if(!out)return 0;
   p.cleanUrl=out.dataUrl;p.cleanBase64=out.base64;p.cleanType=out.mediaType;p.removedText=out.removed;return 1;
  }catch(e){return 0}
 }));
 if(run!==autoRun)return;
...
async function cutoutPhotos(run){
 const targets=product.photos.filter(p=>['packshot','flat','main'].includes(p.role)&&!p.cutUrl&&!p.sourceRegion).slice(0,3);
 if(!targets.length)return '';
 let why='';
 const results=await Promise.all(targets.map(async p=>{
  try{const r=await cutout(p.url,getJSON);
   if(r.ok){p.cutUrl=r.dataUrl;p.cutRatio=r.ratio;return 1}
   why=r.reason;return 0;
  }catch(e){why=e.message;return 0}
 }));
 if(run!==autoRun)return '';
 const done=results.reduce((a,b)=>a+b,0);
...
/* 기본 화질. 실측 — low 17초 / medium 39초 / high 109초.
...
async function generationSource(photo){
 ...
 if(photo.cleanBase64.length<700000)return {base64:photo.cleanBase64,mediaType:photo.cleanType};
...
async function generateImage(plan,run,q){
 ...
 // Complete concepts include raster typography; retain the source reference separately.
...
async function startAutomatic(raw,fromGrab=false,useCurrent=false){
 ...
 try{
  ...
  if(product.photos.some(p=>p.regions?.length)){
   autoMessage('상세이미지에서 사진을 꺼내는 중…');
   const sourcePhotos=product.photos;
   const extracted=await extractPhotoRegions(sourcePhotos,getJSON,()=>run===autoRun);
   if(run!==autoRun)return;
   product.sourcePhotos=sourcePhotos;product.photos=extracted.photos;
   product.regionFailures=extracted.failures;
   PHOTOS.splice(0,PHOTOS.length,...product.photos);
   photoNotice+=` · 상세 사진 ${extracted.extracted}컷 추출`;
...
$('#hqAll') && ($('#hqAll').onchange=e=>{quality=e.target.checked?'high':'medium'});
...
if($('#openSaved'))$('#openSaved').onclick=()=>{if($('#savedStep').hidden){go(3);syncSavedToggle()}else closeSaved()};
...
const completeFill=fillEditor;fillEditor=function(){completeFill();const baked=!!variants[selected]?.completeBanner;for(const id of ['mainCopy','subCopy','ctaCopy','layout','showCta','varyCopy','varyScene'])$('#'+id).disabled=autoBusy||baked;$('#editTitle').title=baked?'텍스트가 포함된 완성 이미지입니다. 수정용 이미지는 광고 글자 제거로 만드세요.':'';};
